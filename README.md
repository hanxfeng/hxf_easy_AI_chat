...
我写的世界观由哔哩哔哩up主艾云的黄钻天空的[超全《明日方舟》剧情党入坑指南-世界观及基础概念篇](https://www.bilibili.com/video/BV1dQ4y137Zu/) 修改得到。
...
v1.11.3  
更新内容：  
为本地版的web端和app端增加了读取聊天记录功能，现在可以网页端打开时会自动加载聊天记录，app端在点击加载聊天记录后会加载之前的聊天记录。并且保存好服务器设置且服务器处于运行状态时进入app会自动读取历史记录。  
修改了web端和app端的气泡，现在更明显了。   
将app的打包方式从云打包换成了使用Android Studio进行本地打包，将app端分开的三种文件合并为单一的html文件，相关源文件也已上传，优化了提示词和聊天记录的储存逻辑。  

零零碎碎的小更新：
修改了一点提示词，现在AI会生成多句的长段了，此时会将这个长段按句号进行输出，也就是说由三句话组成的长短，会发送三次消息，调整了等待时间，按每字一秒增加了每句话之间的停顿时间  
现在AI会有5%的几率将一整段话原样发出而不是一句句发出。  
优化了代码结构，将功能相近的代码进行了合并。  
之前错误的精简代码，导致公网端无法使用，现在已经解决了这个问题。
