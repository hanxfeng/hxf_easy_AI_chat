...
无  
更新内容：  
断断续续的小更新就放在这里了，等攒的更新内容够多就是一个新版本了  
为本地版的web端和app端增加了读取聊天记录功能，现在可以网页端打开时会自动加载聊天记录，app端在点击加载聊天记录后会加载之前的聊天记录。
为本地版的web端的对话框增加了颜色，更方便区分了。
重写了前端页面，现在前端页面更加简洁美观了，为前端页面增加了读取历史记录功能，app端也添加了该功能，不过我打算在重构app端的ui后再进行测试  
将app的打包方式从云打包换成了使用Android Studio进行本地打包，测试更加方便了，优化了提示词和聊天记录的储存逻辑  
